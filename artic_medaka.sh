...
USAGE="USAGE $0 -f sample_manifest_file.csv -r run_name -q input_fastq_files -s path/to/primer/scheme -n normalisation -t threads -k bam"
...
delete_all(){
    for ((i=0; i<${#intermediate_files[@]}; i++)); do
        find . -name "${intermediate_files[$i]}" -type f -delete || continue
    done
}
...
keep_bam(){
    for ((i=0; i<${#intermediate_files_less_bam[@]}; i++)); do
        find . -iname "${intermediate_files_less_bam[$i]}" -type f -delete || continue
    done
}

while getopts f:r:q:s:n:t:k:h opt
do
    case "$opt" in
        f) SAMPLE_NAMES="$OPTARG";;
        r) RUN_NAME="$OPTARG";;
        q) FASTQ_FILES="$OPTARG";;
        s) SCHEME_DIR="$OPTARG";;
